...
import './Cart.css';
 ...
 const Cart = ({product}) => {
    ...
    if (product.quantity === 0  || product.quantity === undefined)  {
       cartEmpty = <p className='cart-empty-message'>Your cart is empty.</p>
  }
  return (
    <div className='cart-information'>
       ...
       <div className='product-to-buy'>
        ...
        <div className='basket-item-information'>
             <p>{product.name}</p>
             <p>{`$ ${product.price} x ${product.quantity}`}<span>  ${product.price * product.quantity}.00</span></p>
